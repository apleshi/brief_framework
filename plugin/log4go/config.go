...
func xmlToFileLogWriter(filename string, props []xmlProperty, enabled bool) (*FileLogWriter, bool) {
	...
	//add mkdir here to make sure log file can be created
	...
	if index >= 0 {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "MkdirAll error, Path = %s, err = %v", d, err)
		}
	}

	//flw := NewFileLogWriter(file, rotate)
	flw := NewFileLogWriter(file, false) //for not to create a file everytime, modify here @xinshaohui
